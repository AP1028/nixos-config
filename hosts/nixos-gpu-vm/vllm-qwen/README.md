# vLLM Qwen3.8-27B-FP8 on gpu-vm (2x RTX 2080 Ti 22GB)

Serves **Qwen/Qwen3.8-27B-FP8** on `nixos-gpu-vm` with the
...
- Runtime tree: `~/vLLM-2080Ti-Definitive` (built with `./build.sh` in `.venv`);
  copy both from the old gpu-host disk onto the VM after first boot
- The llama.cpp RPC cluster was retired with the old vm host; nothing here
...
```sh
# on nixos-gpu-vm, as tianyixia
cd ~/nixos-config/hosts/nixos-gpu-vm/vllm-qwen
...
This directory is imported by `hosts/nixos-gpu-vm/default.nix` as a NixOS
module. The module:
